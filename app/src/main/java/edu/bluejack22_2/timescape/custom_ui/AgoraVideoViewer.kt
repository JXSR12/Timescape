...
import edu.bluejack22_2.timescape.custom_ui.AgoraRtmController.AgoraRtmChannelHandler
import edu.bluejack22_2.timescape.custom_ui.AgoraRtmController.AgoraRtmClientHandler
import edu.bluejack22_2.timescape.custom_ui.AgoraRtmController.AgoraRtmController
import edu.bluejack22_2.timescape.custom_ui.AgoraRtmController.DeviceType
import edu.bluejack22_2.timescape.custom_ui.AgoraRtmController.sendMuteRequest
...
open class AgoraVideoViewer : FrameLayout {
    ...
    internal fun addUserVideo(userId: Int): AgoraSingleVideoView {
        ...
        val docRef = db.collection("liveRooms").document(projectId)

        docRef.get().addOnCompleteListener { task ->
            if (task.isSuccessful) {
                val document = task.result
                if (document.exists()) {
                    val uidMapping = document.get("uidMapping") as? Map<String, Int>
                    if (uidMapping != null) {
                        for ((stringUserId, mappedUid) in uidMapping) {
                            if (mappedUid == userId) {
                                // We have the string user ID, now get the displayName
                                db.collection("users").document(stringUserId).get()
                                        .addOnSuccessListener { userDoc ->
                                            val displayName = userDoc.getString("displayName") ?: "[Unknown User]"
                                            val remoteVideoView =
                                                    AgoraSingleVideoView(this.context, userId, this.agoraSettings.colors.micFlag, stringUserId, displayName)
                                            ...
                                            // After all the setting up, store this user's video view
                                        ...
                                        .addOnFailureListener { exception ->
                                            ...
                                            Log.e("AgoraVideoViewer", "Error fetching user data", exception)
                                        }
        ...
        // return a dummy AgoraSingleVideoView in case Firestore call fails.
        // You might want to handle this differently based on your application's requirements.
        return AgoraSingleVideoView(
                this.context, userId, this.agoraSettings.colors.micFlag, "[Unknown User ID]", "[Unknown User]"
        )
    }
...
//        val canView = userSingleView.hostingView ?: return
    ...
    internal fun addLocalVideo(): AgoraSingleVideoView? {
        ...
        val vidView = AgoraSingleVideoView(this.context, 0, this.agoraSettings.colors.micFlag, stringUserId, userName)
